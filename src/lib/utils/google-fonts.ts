...
export function loadGoogleFont(fontFamily: string, weights: string[] = ["400", "500", "600", "700"]) {
  // Check if font is already loaded
  const existingLink = document.querySelector(`link[href*="family=${fontFamily.replace(/\s+/g, '+')}"]`);
  if (existingLink) {
...
export function preloadGoogleFonts(fonts: string[]) {
  return Promise.all(
    fonts.map(font => loadGoogleFont(font))
  );
}
