...
export function sanitizeVariants(variants: MotionSafeVariants): MotionSafeVariants {
...
export function useMotionSafeColors() {
  ...
  const createColorVariants = React.useCallback((
    lightColor: ColorKey | string,
    darkColor?: ColorKey | string
  ) => {
    const resolvedColor = typeof lightColor === 'string' 
      ? lightColor 
      : getColor(lightColor);
      
    const resolvedDarkColor = darkColor 
      ? (typeof darkColor === 'string' ? darkColor : getColor(darkColor))
      : resolvedColor;
  ...
  const safeBackgroundColor = React.useCallback((
    ...
    if (typeof color === 'string' && color.includes('var(--')) {
      console.warn(`CSS variable in backgroundColor: ${color}, using fallback`);
      return fallback;
    }
...
export const motionSafePresets = {
  ...
  buttonHover: (baseColor: string = 'rgba(0, 0, 0, 0.05)') => ({
    whileHover: { 
      backgroundColor: baseColor === 'transparent' ? 'rgba(0, 0, 0, 0.05)' : baseColor 
    },
    whileTap: { 
      scale: 0.98,
      backgroundColor: baseColor === 'transparent' ? 'rgba(0, 0, 0, 0.1)' : baseColor 
    },
  }),
} as const;
