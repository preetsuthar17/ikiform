...
const COMMON_FILE_TYPES = [
  ...
  { label: 'Documents', value: 'application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document', extensions: ['pdf', 'doc', 'docx'] },
  { label: 'Spreadsheets', value: 'application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', extensions: ['xls', 'xlsx'] },
  { label: 'Videos', value: 'video/*', extensions: ['mp4', 'avi', 'mov', 'wmv'] },
  { label: 'Audio', value: 'audio/*', extensions: ['mp3', 'wav', 'flac', 'm4a'] },
  { label: 'Archives', value: 'application/zip,application/x-rar-compressed,application/x-7z-compressed', extensions: ['zip', 'rar', '7z'] },
];
...
export function FileFieldSettings({ field, onUpdateSettings }: FileFieldSettingsProps) {
  ...
  const updateAcceptAttribute = (types: string[]) => {
    ...
    const fullAccept = acceptTypes || 'image/*,application/pdf,video/*,audio/*,text/*,application/zip';
  ...
  const toggleCommonType = (typeConfig: typeof COMMON_FILE_TYPES[0]) => {
    const hasAllExtensions = typeConfig.extensions.every(ext => allowedTypes.includes(ext));
    
    let newTypes: string[];
    if (hasAllExtensions) {
      // Remove all extensions of this type
      newTypes = allowedTypes.filter(type => !typeConfig.extensions.includes(type));
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            max="50"
            value={maxFiles}
            onChange={(e) => updateSetting('maxFiles', parseInt(e.target.value) || 1)}
            placeholder="10"
          />
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input
              ...
              onChange={(e) => updateSetting('maxSize', (parseInt(e.target.value) || 1) * 1024 * 1024)}
              placeholder="50"
              className="flex-1"
            />
            <span className="flex items-center text-sm text-muted-foreground px-3">MB</span>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                const input = document.getElementById('customType') as HTMLInputElement;
        ...
        {allowedTypes.length > 0 && (
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">Selected extensions:</p>
