-- Storage Setup for Ikiform (Fixed for Supabase permissions)
...
DROP POLICY IF EXISTS "Users can upload files to their forms" ON storage.objects;
DROP POLICY IF EXISTS "Anonymous users can upload files to public forms" ON storage.objects;
DROP POLICY IF EXISTS "Users can view files from their forms" ON storage.objects;
DROP POLICY IF EXISTS "Users can view files from published forms they have access to" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete files from their forms" ON storage.objects;
DROP POLICY IF EXISTS "Users can update files from their forms" ON storage.objects;
...
-- Policy 3: Allow authenticated users to view files from published forms they have access to
